...
esp_err_t ipc_wifi_set_username(uint8_t *username, uint8_t len)
{
    ...
    ipc_msg_buf[1] = len;
    strncpy((void *) &(ipc_msg_buf[2]), (void *) username, len);

    ipc_send(ipc_msg_buf, len + 2);
    return ESP_OK;
}
...
esp_err_t ipc_wifi_set_identity(uint8_t *identity, uint8_t len)
{
    ...
    ipc_msg_buf[1] = len;
    strncpy((void *) &(ipc_msg_buf[2]), (void *) identity, len);
...
esp_err_t ipc_wifi_set_password(uint8_t *password, uint8_t len)
{
    ...
    ipc_msg_buf[1] = len;
    strncpy((void *) &(ipc_msg_buf[2]), (void *) password, len);
...
esp_err_t ipc_wifi_set_ssid(uint8_t *ssid, uint8_t len)
{
    ...
    ipc_msg_buf[1] = len;
    strncpy((void *) &(ipc_msg_buf[2]), (void *) ssid, len);
...
{
    ipc_msg_buf[0] = IPC_API_USERNAME;
    ipc_msg_buf[1] = len;
    strncpy((void *) &(ipc_msg_buf[2]), (void *) username, len);
...
{
    ipc_msg_buf[0] = IPC_API_PASSWORD;
    ipc_msg_buf[1] = len;
    strncpy((void *) &(ipc_msg_buf[2]), (void *) password, len);
...
esp_err_t ipc_handle_msg(uint8_t *msg, uint8_t len, uint8_t msg_id)
{
    switch (msg[0])
    {
        ...
        case IPC_WIFI_USERNAME:
            wifi_set_username(&(msg[2]), msg[1]);
            break;

        case IPC_WIFI_IDENTITY:
            wifi_set_identity(&(msg[2]), msg[1]);
            break;

        case IPC_WIFI_PASSWORD:
            wifi_set_password(&(msg[2]), msg[1]);
            break;

        case IPC_WIFI_SSID:
            wifi_set_ssid(&(msg[2]), msg[1]);
            break;
        ...
        case IPC_API_USERNAME:
            wifi_set_api_username(&(msg[2]), msg[1]);
            break;

        case IPC_API_PASSWORD:
            wifi_set_api_password(&(msg[2]), msg[1]);
            break;
